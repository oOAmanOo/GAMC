...
class Generator(nn.Module):
    ...
    def forward(self, image, text_id, mode):
        ...
        ########################################### feature fusion ###########################################
        if mode == 'train':
            dummy_token = torch.full((text_id.shape[0], 64), 0, dtype=text_id.dtype).to(device)
    ...
    def generate_withText(self, image, text_id, trainTest):
        if trainTest == 'test':
            dummy = torch.full_like(text_id, 0, dtype=text_id.dtype).to(device)
            text_embedd = self.gemma.model.embed_tokens(dummy)
        else:
            text_embedd = self.gemma.model.embed_tokens(text_id)
        ##############################################   generate ##############################################
    ...
    def print(self):
        def dataframe_Name(name, count=192, rows=1):
            if rows == 1:
                ...
                name_df = pd.DataFrame([[name] * count for _ in range(rows)], columns=[f"{i}" for i in range(128, 192)])
                return name_df

        test = pd.DataFrame()
        test['Name'] = ['train1', 'train2', 'train3', 'train4', 'train5', 'train6', 'train7', 'train8', 'train9',
                        'train10']
        Only_image_df = pd.DataFrame(self.Only_image.cpu().detach(), columns=[f"{i}" for i in range(0, 128)])
...
#######################################
checkpoint_folder = '20241219_Clip_Clip_Clip_noGemma_testNoText_img2txt'
