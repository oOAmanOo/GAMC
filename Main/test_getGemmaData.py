...
class OxfordDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # imageData = torch.load('../../Instagram/ImageData/wendys/'+ self.image[idx] +'.pt', weights_only=False)
        # imageData = torch.load('../../Oxford_HIC/ImageBeit/'+ self.image[idx] +'.pt', weights_only=False)
        ...
        return self.text[idx], imageData, self.image[idx]
...
def train():
    ...
    def getData(logits, caption, image, tokenizer, trainTest, file, name):
        top_k_values, top_k_indices = torch.topk(logits, 1, dim=2, largest=True)
        ...
        reverse_data = insert_zeros(top_k_indices.squeeze(-1))
        ...
        if not os.path.exists('../Data/SFTTrainer/'+ file):
            os.makedirs('../Data/SFTTrainer/'+ file)
        if os.path.exists('../Data/SFTTrainer/'+ file+'/' + name + '.csv'):
            data1 = pd.read_csv('../Data/SFTTrainer/'+ file+'/' + name + '.csv')
        else:
            data1 = pd.DataFrame(columns=['prompt', 'label', 'image'])
        data2 = pd.DataFrame(reverse, columns=['prompt'])
        data2['label'] = caption
        data2['image'] = image
        data1 = pd.concat([data1, data2], axis=0)
        data1.to_csv('../Data/SFTTrainer/'+ file+'/' + name + '_'+trainTest+'.csv', index=False)
    ...
    file = '20241209_SFTTrainer'
    name = '20241209_0_noPrompt'
    with tqdm(train_loader, unit="batch", leave=True) as tepoch:
        for idx, (text, image_embedd, image) in enumerate(tepoch):
            text_id = textExtraction(tokenizer, gemmaConfig, text)
            image_embedd = image_embedd.to(torch.bfloat16)
            logits = Generator(NetFormer, image_embedd.to(device), text_id.to(device))
            getData(logits, text, image, tokenizer, 'train', file, name)
    with tqdm(test_loader, unit="batch", leave=True) as tepoch:
        for idx, (text, image_embedd, image) in enumerate(tepoch):
            text_id = textExtraction(tokenizer, gemmaConfig, text)
            image_embedd = image_embedd.to(torch.bfloat16)
            logits = Generator(NetFormer, image_embedd.to(device), text_id.to(device))
            getData(logits, text, image, tokenizer, 'test', file, name)
