...
class ClipCocoDataset(Dataset):
    ...
    def filter_data_by_bleu(self, model, batch_size=20, bleu_threshold=0.1, file_path=None):
        ...
        for batch in dataloader:
            ...
            # text_embeds = model.falcon.model.embed_tokens(tokens_batch)
            # embedding_cat = torch.cat((prefix_embeds, text_embeds), dim=1)
            outputs = model.falcon(inputs_embeds=prefix_embeds, attention_mask=masks_batch)
        ...
        self.captions_tokens = [self.captions_tokens[i] for i in filtered_indices]
        self.caption2embedding = [self.caption2embedding[i] for i in filtered_indices]
        del tokens_batch, masks_batch, prefixes_batch, original_indices, prefix_embeds, outputs, logits, generated_tokens_batch, reference, candidate, bleu_score
        gc.collect()
        torch.cuda.empty_cache()
...
class TransformerMapper(nn.Module):
    ...
    def forward(self, x):
        if self.mode == 'text':
            ...
            prefix1 = torch.cat((x, prefix), dim=1)
            prefix2 = torch.cat((prefix, x), dim=1)
            out1 = self.transformer(prefix1)[:, self.clip_length:]
            out2 = self.transformer(prefix2)[:, :self.clip_length]
            out = torch.cat((out1, out2), dim=1)
            out = self.linearSmall(out.transpose(1, 2)).transpose(1, 2)
            out = self.mlp(out)
            out = self.norm(out)
        return out

    def __init__(self, dim_clip: int, dim_embedding: int, prefix_length: int, clip_length: int, num_layers: int = 8, num_heads: int = 8, mode: str = 'text'):
        ...
        self.linearSmall = nn.Linear(clip_length*2, clip_length)
...
class ClipCaptionModel(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, prefix: torch.Tensor, mask: Optional[torch.Tensor] = None,
        ...
        # text_projections = self.text_project(embedding_text).view(-1, self.prefix_length, self.embedding_size)
        # print(embedding_text.shape, prefix.shape)
        prefix_projections = self.visual_self_project(prefix).view(-1, self.prefix_length, self.embedding_size)
        prefix_projections = self.visual_co_project(prefix_projections).view(-1, self.prefix_length, self.embedding_size)
    ...
    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 512,
                 num_layers: int = 8, mapping_type: MappingType = MappingType.MLP, num_heads: int = 8):
        ...
        #     param.requires_grad = False
        self.visual_self_project = TransformerMapper(prefix_size, self.embedding_size, prefix_length, clip_length, num_layers,num_heads=1, mode='visual')
        self.visual_co_project = TransformerMapper(prefix_size, self.embedding_size, prefix_length, clip_length, num_layers,num_heads=num_heads, mode='coAttention')
        self.text_project = TransformerMapper(prefix_size, self.embedding_size, prefix_length, clip_length,num_layers, num_heads=num_heads, mode='text')
...
def train(trainData, testData, model: ClipCaptionModel, args,
    ...
    model.eval()
    trainDataset = OxfordDataset(trainData, prefix_length, normalize_prefix, model=model, batch_size=bleu_batch_size,
                                   bleu_threshold=bleu_threshold)
    testDataset = OxfordDataset(testData, prefix_length, normalize_prefix, model=model, batch_size=bleu_batch_size,
                                  bleu_threshold=bleu_threshold)
    ...
    while len(trainDataset) > batch_size and len(testDataset) > batch_size:
        ...
        with torch.no_grad():
            ...
            for idx, (tokens, mask, prefix, original_indices) in enumerate(test_dataloader):
                ...
                prefix_projections = model.visual_self_project(prefix).view(-1, model.prefix_length, model.embedding_size)
                prefix_projections = model.visual_co_project(prefix_projections).view(-1, model.prefix_length, model.embedding_size)
                outputs = model.falcon(inputs_embeds=prefix_projections, attention_mask=mask)
                logits = outputs.logits
                generated_tokens = torch.argmax(logits, dim=-1)
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--trainData', default='../Data/Oxford_HIC/parse/oxford_only10_ViT-B_32-2layer_train.pkl')
    parser.add_argument('--testData', default='../Data/Oxford_HIC/parse/oxford_only10_ViT-B_32-2layer_test.pkl')
    parser.add_argument('--out_dir', default='20250121_totalClip_oxford_only10_transformer_p64concatimg_falcon_swin_tf8_mine')
    ...
    if args.only_prefix:
        model = ClipCaptionPrefix(prefix_length, clip_length=prefix_length, prefix_size=prefix_dim, num_layers=args.num_layers, mapping_type=args.mapping_type, num_heads=args.num_heads)
        print("Train only prefix")
    else:
        model = ClipCaptionModel(prefix_length, clip_length=prefix_length, prefix_size=prefix_dim, num_layers=args.num_layers, mapping_type=args.mapping_type, num_heads=args.num_heads)
        print("Train both prefix and GPT")
        sys.stdout.flush()
    device = torch.device('cuda:0')
    model = model.to(device, dtype=torch.bfloat16)
    # 20250115_totalClip_oxford_only100_300k_transformer_p40_falcon_bleu1_0.05 == 32
    # save_file = '20250115_totalClip_oxford_only100_300k_transformer_p40_falcon_bleu1_0.1'
    # i = 7
    # model.load_state_dict(torch.load(f'./Model/{save_file}/checkpoint-{i:03d}.pt'))
    model.eval()
    train(args.trainData, args.testData, model, args, output_dir=args.out_dir, output_prefix=args.prefix , bleu_batch_size=20, bleu_threshold=0.1)
