...
class ImageTextModel(nn.Module):
    ...
    def __init__(self, gpt2_model_name="gpt2", feature_dim=768, output_dim=1):
        ...
        self.resnet = models.resnet50(pretrained=True) # full model
        #### Option 1: -1 layer ###########
    ...
    with torch.no_grad():
        for n in range(input_num):
            ...
            if dataPath != "none":
                ...
                ###############  test all results  (pred.test)  ##################
                image_list = data['image_id'].tolist()
                #####   test 10 train/test samples from GAMC (pred.predict)  #####
                # image_list = []
                # image_list.append('')
