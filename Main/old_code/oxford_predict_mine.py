...
class Predictor(object):
    ...
    def predict(self, tokens, masks, prefixs, text_gt, model):
        ...
        for i, text in enumerate(text_gt):
            ...
            print(text)
            print('--------- generate_beam ---------')
            prefix_projections = model.visual_self_project(prefixs[i].unsqueeze(0)).view(-1, self.prefix_length, self.embedding_size)
            prefix_projections = model.visual_co_project(prefix_projections).view(-1, self.prefix_length, self.embedding_size)
            caption = generate_beam(model, tokenizer, embed=prefix_projections)[0]
            ...
            print('---------generate2---------')
            caption = generate2(model, tokenizer, embed=prefix_projections)
            caption_token = tokenizer(caption, return_tensors='pt', padding='max_length', truncation=True, max_length=74)
            print(caption)
            ...
            print(prefix_projections.shape, embedding_text.shape)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
trainData = '../Data/Oxford_HIC/parse/oxford_only10_ViT-B_32-2layer_train.pkl'
testData = '../Data/Oxford_HIC/parse/oxford_only10_ViT-B_32-2layer_test.pkl'
prefix_length = 64
normalize_prefix = False
...
save_file = '20250121_totalClip_oxford_only10_transformer_p64concatimg_falcon_swin_tf8_mine'
for i in range(5):
