...
def train():
    ...
    class Former(nn.Module):
        def __init__(self, depth=12):
            ...
            # self.image_proj = nn.Linear(512, 64*768)
        ...
        def forward(self, text, image):
            ...
            # image = image.view(image.shape[0], 64, 768)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, unit="batch", leave=True) as tepoch:
            for idx, (text, image, funny_score) in enumerate(tepoch):
                text,_ = textExtraction(tokenizer, gemmaConfig, text, 'train')
