...
class Generator(nn.Module):
    ...
    def generate_withText(self, image, text_id, mode, mask=None):
        ...
        gptOutput = self.gpt(inputs_embeds=image_GG, attention_mask=mask)
        logits = gptOutput.logits
        caption_tokens = logits.argmax(-1)[0]
        caption = tokenizer.decode(caption_tokens, skip_special_tokens=True, clean_up_tokenization_spaces=False)
        print(caption)
    ...
    def print(self):
        def dataframe_Name(name, count=74, rows=1):
            if rows == 1:
                ...
                name_df = pd.DataFrame([[name] * count for _ in range(rows)], columns=[f"{i}" for i in range(74-count, 74)])
                return name_df
        ...
        test['Name'] = ['test1', 'test2','train1', 'train2', 'train3', 'train4', 'train5', 'train6', 'train7', 'train8', 'train9',
                        'train10']
        Only_image_df = pd.DataFrame(self.Only_image.cpu().detach(), columns=[f"{i}" for i in range(0, 10)])
        ...
        Empty_Text_df = pd.DataFrame(self.Empty_Text.cpu().detach(), columns=[f"{i}" for i in range(0, 74)])
        ...
        Full_Text_df = pd.DataFrame(self.Full_Text.cpu().detach(), columns=[f"{i}" for i in range(0, 74)])
...
#######################################
checkpoint_folder = '20241221_Clip_Clip_Clip_noGPT_prefix10_mask_Adam'
checkpoint_Generator = './Model/' + checkpoint_folder + '/' + checkpoint_folder + '_NetLLM_16.pth'
#######################################
...
def loss_function_10(logits, labels):
    ...
    loss = nn.functional.cross_entropy(logits, labels, ignore_index=50265)
    return loss
...
text_data = tokenizer(test_gt, return_tensors='pt', padding='max_length', truncation=True, max_length=10)
...
text_id = text_data['input_ids'].to(device)
mask = torch.cat((torch.ones(1, 10), text_data['attention_mask']), dim=1).to(device)
print("========= Empty Text =========")
